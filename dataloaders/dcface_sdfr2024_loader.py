...
from utils.os_utils import get_all_files
import torch
import cv2
from torch.utils.data import Dataset, DataLoader
import numpy as np
...
class BaseMXDataset(Dataset):
    def __init__(self, root_dir, swap_color_order=False, resolution=112):
        ...
        # path_imgrec = os.path.join(root_dir, 'train.rec')  # original
        # path_imgidx = os.path.join(root_dir, 'train.idx')
        # path_imglst = os.path.join(root_dir, 'train.lst')
        path_imgrec = os.path.join(root_dir, 'file.rec')     # Bernardo
        path_imgidx = os.path.join(root_dir, 'file.idx')
        path_imglst = os.path.join(root_dir, 'list.txt')
